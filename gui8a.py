...
for window in ahk.windows():
    ...
    # need to add the line here where text matches the inmate addtnl form (from reports menu)
...
def lencheck(fac, fn, ln, inm):
    #print(len(fn))
    assert (len(fac) <= 40), "Facility name is too long; 40 char limit."
...
while True:
    ...
    if event == 'Submit':
        if values['fn'] == '' or values['ln'] == '' or values['inm'] == '' or values['subproject'] == '':
            ...
            # for next function > , values['fn'], values['ln'], values['inm']
            ret = fieldentry(values['subproject'])
            mailwarefocus(ret['name'], values['fn'], values['ln'], values['inm'])
