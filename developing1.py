import tkinter as tk
import webbrowser


def show_entry_fields():
    print("First Name: %s\nLast Name: %s" % (e1.get(), e2.get()))

master = tk.Tk()
tk.Label(master, 
         text="First Name").grid(row=0)
tk.Label(master, 
         text="Last Name").grid(row=1)

e1 = tk.Entry(master)
e2 = tk.Entry(master)

e1.grid(row=0, column=1)
e2.grid(row=1, column=1)

tk.Button(master, 
          text='Quit', 
          command=master.quit).grid(row=3, 
                                    column=0, 
                                    sticky=tk.W, 
                                    pady=4)
tk.Button(master, 
          text='Show', command=show_entry_fields).grid(row=3, 
                                                       column=1, 
                                                       sticky=tk.W, 
                                                       pady=4)

tk.mainloop()

#TESTING below

states = {
    "MD" : "www.google.com"
}

def find_state():
    entry = e1.get()
    if entry not in states:
      print("Sorry, state not found. Enter abbreviation as 2 letter capitals only.")
    else:
      url = states.get(e1.get())
      webbrowser.open(url, new=0, autoraise=True)

master = tk.Tk()
tk.Label(master, 
         text="Which state are you looking for? Enter 2-letter abbreviation, ie MD").grid(row=0)

e1 = tk.Entry(master)

e1.grid(row=0, column=1)

tk.Button(master, 
          text='Quit', 
          command=master.quit).grid(row=3, 
                                    column=0, 
                                    sticky=tk.W, 
                                    pady=4)
tk.Button(master, 
          text='Press Enter to Find State Info', command=find_state).grid(row=3, 
                                                       column=1, 
                                                       sticky=tk.W, 
                                                       pady=4)

tk.mainloop()






nums = [1, 23, 4]

for n in nums:
    print (n+1)
    
    
states = {
    "MD" : "www.google.com"
}


while True:
    try:
        q = input("Which state are you looking for? Enter 2-letter abbreviation, ie MD")
        if q in states:
            print("found!")
            print(states.get(q))
            url = states.get(q)
            webbrowser.open(url, new=0, autoraise=True)
        elif q == 'exit':
            break
        else:
            print("sorry not found, try again.")
    except Exception as e:
        print(e)
        print("sorry didn't work, see yc to debug iA")
        
"""        
'AL' : ''
'AK' : ''
'AZ' : ''
'AR' : ''
'CA' : ''
'CO' : ''
'CT' : ''
'DE' : ''
'FL' : ''
'GA' : ''
'HI' : ''
'ID' : ''
'IL' : ''
'IN' : ''
'IA' : ''
'KS' : ''
'KY' : ''
'LA' : ''
'ME' : ''
'MD' : ''
'MA' : ''
'MI' : ''
'MN' : ''
'MS' : ''
'MO' : ''
'MT' : ''
'NE' : ''
'NV' : ''
'NH' : ''
'NJ' : ''
'NM' : ''
'NY' : ''
'NC' : ''
'ND' : ''
'OH' : ''
'OK' : ''
'OR' : ''
'PA' : ''
'RI' : ''
'SC' : ''
'SD' : ''
'TN' : ''
'TX' : ''
'UT' : ''
'VT' : ''
'VA' : ''
'WA' : ''
'WV' : ''
'WI' : ''
'WY' : ''
"""

#below works!!!!!! <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<,
import tkinter as tk
from tkinter import ttk
import webbrowser
import time


LARGE_FONT= ("Verdana", 12)
NORM_FONT = ("Helvetica", 10)
SMALL_FONT = ("Helvetica", 8)

states = {
    'MD' : 'www.google.com'
}


def popupmsg(msg):
    popup = tk.Tk()
    popup.wm_title("!")
    label = ttk.Label(popup, text=msg, font=NORM_FONT)
    label.pack(side="top", fill="x", pady=10)
    B1 = ttk.Button(popup, text="Okay", command = popup.destroy)
    B1.pack()
    popup.after(3000, lambda: popup.destroy())
    popup.mainloop()
    
#command = lambda: popupmsg("popup message here!"))


def find_state():
    entry = e1.get()
    if entry not in states:
        e1.delete(0, tk.END)
        msg = "Sorry, state not found. Enter abbreviation as 2 letter capitals only."
        popupmsg(msg)
        master.quit()

    else:
        state = e1.get()
        url = states.get(state)
        webbrowser.open(url, new=0, autoraise=True)
        e1.delete(0, tk.END)

master = tk.Tk()
tk.Label(master, 
         text="Which state are you looking for? Enter 2-letter abbreviation, ie MD").grid(row=0)

e1 = tk.Entry(master)

e1.grid(row=0, column=1)

def process(event=None):
    find_state()
e1.bind('<Return>', process)

tk.Button(master, 
          text='Quit', 
          command=master.quit).grid(row=3, 
                                    column=0, 
                                    sticky=tk.W, 
                                    pady=4)
tk.Button(master, 
          text='Press Enter to Find State Info', command=find_state).grid(row=3, 
                                                       column=1, 
                                                       sticky=tk.W, 
                                                       pady=4)

tk.mainloop()

#above works!!!!!>>>>>>>>>>>>>>>>>>


"""
def show_entry_fields():
    print("First Name: %s\nLast Name: %s" % (e1.get(), e2.get()))

master = tk.Tk()
tk.Label(master, 
         text="First Name").grid(row=0)
tk.Label(master, 
         text="Last Name").grid(row=1)

e1 = tk.Entry(master)
e2 = tk.Entry(master)

e1.grid(row=0, column=1)
e2.grid(row=1, column=1)

tk.Button(master, 
          text='Quit', 
          command=master.quit).grid(row=3, 
                                    column=0, 
                                    sticky=tk.W, 
                                    pady=4)
tk.Button(master, 
          text='Show', command=show_entry_fields).grid(row=3, 
                                                       column=1, 
                                                       sticky=tk.W, 
                                                       pady=4)

tk.mainloop()
"""



"""
def show_entry_fields():
    print("First Name: %s\nLast Name: %s" % (e1.get(), e2.get()))

master = tk.Tk()
tk.Label(master, 
         text="First Name").grid(row=0)
tk.Label(master, 
         text="Last Name").grid(row=1)

e1 = tk.Entry(master)
e2 = tk.Entry(master)

e1.grid(row=0, column=1)
e2.grid(row=1, column=1)

tk.Button(master, 
          text='Quit', 
          command=master.quit).grid(row=3, 
                                    column=0, 
                                    sticky=tk.W, 
                                    pady=4)
tk.Button(master, 
          text='Show', command=show_entry_fields).grid(row=3, 
                                                       column=1, 
                                                       sticky=tk.W, 
                                                       pady=4)

tk.mainloop()
"""